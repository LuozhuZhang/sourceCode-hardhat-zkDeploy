...
// * hardhat中的Artifact用于部署一个合约，zk-deployer继承了这个方法
// * Artifact in hardhat is used to deploy a contract, zk-deployer inherits this method
export interface Artifact {
  _format: string;
  contractName: string;
  sourceName: string;
  // * abi和bytecode非常重要
  // * Abi and bytecode are very important
  abi: any[];
  bytecode: string; // "0x"-prefixed hex string
  deployedBytecode: string; // "0x"-prefixed hex string
  // * 当前智能合约所依赖的其他智能合约的地址
  // * Addresses of other smart contracts that the current smart contract depends on
